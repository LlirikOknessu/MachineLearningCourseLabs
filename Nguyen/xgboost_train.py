...
if __name__ == '__main__':
    ...
    output_model_joblib_path = out_dir / 'xgboost.joblib'
    ...
    random.seed(42)
    xgb_reg = GridSearchCV(xgb.XGBRegressor(), param_grid=params, scoring='neg_root_mean_squared_error', n_jobs=-1, cv=10, verbose=0)
    xgb_reg = xgb_reg.fit(X=X_train, y=y_train)    
    print(xgb_reg.best_params_)
    predicted_values = np.squeeze(xgb_reg.predict(X_val))
