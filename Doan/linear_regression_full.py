...
if __name__ == '__main__':
    ...
    y_mean = y_full.mean()
    y_std = y_full.std()
    y_pred_baseline = np.random.normal(y_mean, y_std, len(y_full))

    predicted_values = np.squeeze(reg.predict(X_full))


    print(reg.score(X_full, y_full))
    print("Mean posttest value: ", y_full.mean())
    ...
    output_model = pd.DataFrame([coefficients, intercept])
    output_model.to_csv(output_model_path, index=False)
